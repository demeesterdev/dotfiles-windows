function Initialize-WSLDistribution {
    ...
    Invoke-WSLCommand -DistributionName $Distribution -user 'root' -Command "/usr/sbin/adduser --disabled-password --quiet --gecos '' '$Username'"

    Write-Verbose "Distribution $Distribution : user account $UserName - add to relevant groups"
    Invoke-WSLCommand -DistributionName $Distribution -user 'root' -Command "/usr/sbin/usermod -aG adm,cdrom,sudo,dip,plugdev '$Username'"
